<template>
  <div>
    <section
      id="search-panels"
      class="searchPanel search-panels"
      style="background-image:url(/images/hero/tour-bg.jpg);"
      data-v-fd004460
    >
      <div class="searchPanel-cover" data-v-fd004460></div>
      <div class="panels" data-v-fd004460>
        <div class="container" data-v-fd004460>
          <ul class="searchPanel-segment" data-v-fd004460>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a
                href="index.html"
                data-test="domestic-link"
                class="nuxt-link-active"
                data-v-fd004460
              >
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    data-v-fd004460
                  >
                    <path
                      fill="currentColor"
                      d="M32 62.933c-17.084 0-30.933-13.849-30.933-30.933s13.849-30.933 30.933-30.933c17.084 0 30.933 13.849 30.933 30.933s-13.849 30.933-30.933 30.933zM32 64c17.673 0 32-14.327 32-32s-14.327-32-32-32c-17.673 0-32 14.327-32 32s14.327 32 32 32zM47.38 47.995l1.499-5.119c0.449-0.78 0.835-1.636 1.424-2.326 0.679-0.796 1.75-1.882 3.212-3.258 0.213-0.2 0.223-0.536 0.023-0.749-0.035-0.038-0.076-0.070-0.121-0.096l-0.228-0.132c-0.25-0.144-0.547-0.181-0.825-0.102-2.574 0.731-4.525 1.267-5.852 1.607-3.709-2.602-7.483-5.121-11.342-7.508 1.146-1.978 2.557-3.772 3.915-5.609 2.065-2.794 4.152-5.662 6.262-8.602v0c0.261-0.364 0.178-0.87-0.186-1.131-0.022-0.015-0.044-0.030-0.067-0.043-0.47-0.272-1.057-0.237-1.491 0.088-2.57 1.922-4.5 3.414-5.791 4.475l-1.173-0.647c-0.427-0.247-0.945-0.151-1.155 0.213s-0.035 0.86 0.393 1.107l0.65 0.375-2.526 2.060-1.195-0.69c-0.428-0.246-0.945-0.151-1.156 0.213-0.21 0.365-0.034 0.86 0.393 1.107l0.698 0.403c-2.508 2.070-1.255 1.034-3.758 3.109-1.548-0.894-11.339-6.389-13.253-6.371-0.873 0.008-1.796 0.253-2.207 0.965s-0.161 1.633 0.268 2.393c0.666 0.989 1.978 1.82 2.931 2.493 2.961 2.092 6.096 3.942 9.213 5.798-0.208 0.603-0.021-0.007-0.187 1.151-0.176 1.231-0.417 2.446-0.591 3.679l-0.735-0.425c-0.427-0.247-0.944-0.152-1.155 0.213s-0.034 0.86 0.393 1.107l1.24 0.716-0.515 3.221-0.701-0.404c-0.427-0.247-0.945-0.151-1.155 0.213s-0.035 0.86 0.393 1.107l1.218 0.703c-0.25 1.652-0.595 4.066-1.034 7.243-0.074 0.536 0.184 1.063 0.652 1.333 0.388 0.224 0.884 0.091 1.107-0.297 0.013-0.023 0.025-0.046 0.036-0.070l7.225-15.915c3.983 2.235 8.114 4.205 12.218 6.19 0.316 1.303 0.809 3.22 1.479 5.753 0.073 0.275 0.252 0.509 0.498 0.651l0.235 0.136c0.269 0.155 0.612 0.063 0.768-0.206 0.022-0.039 0.040-0.080 0.053-0.123z"
                    />
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>پرواز داخلی</p>
              </a>
            </li>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a href="/iranout" data-test="international-link" data-v-fd004460>
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    data-v-fd004460
                  >
                    <path
                      fill="currentColor"
                      d="M47.992 33.007c0.338-0.093 0.695-0.192 1.071-0.297 0.003-0.236 0.004-0.473 0.004-0.71 0-3.542-0.307-6.949-0.873-10.133h13.042c1.1 3.175 1.698 6.584 1.698 10.133s-0.598 6.959-1.698 10.133h-6.582c-0.408 0.414-0.743 0.77-1.002 1.067h7.192c-3.885 9.997-12.817 17.475-23.668 19.302 3.162-1.885 5.925-5.45 7.989-10.136-0.251-0.093-0.498-0.209-0.738-0.347l-0.219-0.126c-2.855 6.511-7.058 10.721-11.675 11.023v-17.889l-1.067 2.35v15.539c-3.66-0.24-7.059-2.935-9.734-7.286-0.035-0.019-0.071-0.039-0.106-0.060-1.971-1.139-3.056-3.357-2.744-5.612 0.012-0.084 0.023-0.166 0.034-0.249-0.734-2.007-1.348-4.19-1.82-6.51h1.264c-0.1-0.346-0.163-0.704-0.186-1.067h-1.284c-0.576-3.171-0.891-6.579-0.891-10.133 0-0.367 0.003-0.733 0.010-1.097-0.357-0.238-0.706-0.474-1.050-0.712-0.018 0.599-0.027 1.202-0.027 1.809 0 3.542 0.307 6.949 0.873 10.133h-13.042c-1.1-3.175-1.698-6.584-1.698-10.133s0.598-6.959 1.698-10.133h6.263c0.018-0.355 0.068-0.711 0.154-1.067h-6.025c3.885-9.997 12.817-17.475 23.668-19.302-4.136 2.465-7.588 7.805-9.689 14.768 0.323 0.071 0.665 0.165 1.029 0.284 2.717-8.99 7.706-15.099 13.302-15.466v16.484l0.007-0.001c0.088-0.219 0.194-0.436 0.318-0.649 0.214-0.37 0.464-0.699 0.742-0.986v-14.848c4.239 0.278 8.128 3.849 10.947 9.472 0.38-0.064 0.766-0.089 1.152-0.074-1.997-4.119-4.559-7.257-7.457-8.984 10.851 1.828 19.784 9.305 23.668 19.302h-12.851c-0.056-0.278-0.113-0.555-0.172-0.83-0.458 0.636-0.915 1.268-1.371 1.896h0.66c0.576 3.171 0.891 6.579 0.891 10.133 0 0.337-0.003 0.673-0.008 1.007zM33.847 42.133l-0.484 1.067h4.137c-0.778-0.383-1.486-0.734-2.146-1.067h-1.506zM3.157 43.2h12.851c1.825 9.157 5.823 16.326 10.817 19.302-10.851-1.828-19.784-9.305-23.668-19.302zM32 64c17.673 0 32-14.327 32-32s-14.327-32-32-32c-17.673 0-32 14.327-32 32s14.327 32 32 32zM47.38 47.995l1.499-5.119c0.449-0.78 0.835-1.636 1.424-2.326 0.679-0.796 1.75-1.882 3.212-3.258 0.213-0.2 0.223-0.536 0.023-0.749-0.035-0.038-0.076-0.070-0.121-0.096l-0.228-0.132c-0.25-0.144-0.547-0.181-0.825-0.102-2.574 0.731-4.525 1.267-5.852 1.607-3.709-2.602-7.483-5.121-11.342-7.508 1.146-1.978 2.557-3.772 3.915-5.609 2.065-2.794 4.152-5.662 6.262-8.602v0c0.261-0.364 0.178-0.87-0.186-1.131-0.022-0.015-0.044-0.030-0.067-0.043-0.47-0.272-1.057-0.237-1.491 0.088-2.57 1.922-4.5 3.414-5.791 4.475l-1.173-0.647c-0.427-0.247-0.945-0.151-1.155 0.213s-0.035 0.86 0.393 1.107l0.65 0.375-2.526 2.060-1.195-0.69c-0.428-0.246-0.945-0.151-1.156 0.213-0.21 0.365-0.034 0.86 0.393 1.107l0.698 0.403c-2.508 2.070-1.255 1.034-3.758 3.109-1.548-0.894-11.339-6.389-13.253-6.371-0.873 0.008-1.796 0.253-2.207 0.965s-0.161 1.633 0.268 2.393c0.666 0.989 1.978 1.82 2.931 2.493 2.961 2.092 6.096 3.942 9.213 5.798-0.208 0.603-0.021-0.007-0.187 1.151-0.176 1.231-0.417 2.446-0.591 3.679l-0.735-0.425c-0.427-0.247-0.944-0.152-1.155 0.213s-0.034 0.86 0.393 1.107l1.24 0.716-0.515 3.221-0.701-0.404c-0.427-0.247-0.945-0.151-1.155 0.213s-0.035 0.86 0.393 1.107l1.218 0.703c-0.25 1.652-0.595 4.066-1.034 7.243-0.074 0.536 0.184 1.063 0.652 1.333 0.388 0.224 0.884 0.091 1.107-0.297 0.013-0.023 0.025-0.046 0.036-0.070l7.225-15.915c3.983 2.235 8.114 4.205 12.218 6.19 0.316 1.303 0.809 3.22 1.479 5.753 0.073 0.275 0.252 0.509 0.498 0.651l0.235 0.136c0.269 0.155 0.612 0.063 0.768-0.206 0.022-0.039 0.040-0.080 0.053-0.123z"
                    />
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>پرواز خارجی</p>
              </a>
            </li>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a href="/train-ticket" data-test="train-link" data-v-fd004460>
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    width="64"
                    height="64"
                    viewBox="0 0 64 64"
                    data-v-fd004460
                  >
                    <g fill="currentColor" fill-rule="nonzero">
                      <path
                        d="M32 62.933c17.084 0 30.933-13.849 30.933-30.933s-13.849-30.933-30.933-30.933c-17.084 0-30.933 13.849-30.933 30.933s13.849 30.933 30.933 30.933zM32 64c-17.673 0-32-14.327-32-32s14.327-32 32-32c17.673 0 32 14.327 32 32s-14.327 32-32 32zM39.733 42.667h-15.467l-7.84 10.453c-0.177 0.236-0.511 0.283-0.747 0.107s-0.283-0.511-0.107-0.747l7.36-9.813h-1.6c-2.945 0-5.333-2.388-5.333-5.333v-16c0-5.891 4.776-10.667 10.667-10.667h10.667c5.891 0 10.667 4.776 10.667 10.667v16c0 2.946-2.388 5.333-5.333 5.333h-1.6l7.36 9.813c0.177 0.236 0.129 0.57-0.107 0.747s-0.57 0.129-0.747-0.107l-7.84-10.453zM25.6 13.867v0c-3.535 0-6.4 2.865-6.4 6.4v5.333c0 1.178 0.955 2.133 2.133 2.133h21.333c1.178 0 2.133-0.955 2.133-2.133v-5.333c0-3.535-2.865-6.4-6.4-6.4h-12.8zM23.467 38.4c1.767 0 3.2-1.433 3.2-3.2s-1.433-3.2-3.2-3.2c-1.767 0-3.2 1.433-3.2 3.2s1.433 3.2 3.2 3.2zM40.533 38.4c1.767 0 3.2-1.433 3.2-3.2s-1.433-3.2-3.2-3.2c-1.767 0-3.2 1.433-3.2 3.2s1.433 3.2 3.2 3.2zM25.067 48c-0.295 0-0.533-0.239-0.533-0.533s0.239-0.533 0.533-0.533h13.899c0.295 0 0.533 0.239 0.533 0.533s-0.239 0.533-0.533 0.533h-13.899zM20.8 53.333c-0.295 0-0.533-0.239-0.533-0.533s0.239-0.533 0.533-0.533h22.4c0.295 0 0.533 0.239 0.533 0.533s-0.239 0.533-0.533 0.533h-22.4z"
                      />
                    </g>
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>قطار</p>
              </a>
            </li>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a href="/bus-ticket" data-test="bus-link" data-v-fd004460>
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 70 70" data-v-fd004460>
                    <g fill="none" fill-rule="evenodd">
                      <rect width="70" height="70" />
                      <path
                        fill="currentColor"
                        fill-rule="nonzero"
                        d="M35,68.8333333 C53.685634,68.8333333 68.8333333,53.685634 68.8333333,35 C68.8333333,16.314366 53.685634,1.16666667 35,1.16666667 C16.314366,1.16666667 1.16666667,16.314366 1.16666667,35 C1.16666667,53.685634 16.314366,68.8333333 35,68.8333333 Z M35,70 C15.6700338,70 0,54.3299662 0,35 C0,15.6700338 15.6700338,0 35,0 C54.3299662,0 70,15.6700338 70,35 C70,54.3299662 54.3299662,70 35,70 Z M18.6666667,53.6670939 C17.2498122,52.6028532 16.3333333,50.9084541 16.3333333,49 L16.3333333,19.8333333 C16.3333333,14.6786757 20.512009,10.5 25.6666667,10.5 L44.3333333,10.5 C49.487991,10.5 53.6666667,14.6786757 53.6666667,19.8333333 L53.6666667,49 C53.6666667,50.9084541 52.7501878,52.6028532 51.3333333,53.6670939 L51.3333333,57.1666667 C51.3333333,58.4553311 50.2886644,59.5 49,59.5 L46.6666667,59.5 C45.3780023,59.5 44.3333333,58.4553311 44.3333333,57.1666667 L44.3333333,54.8333333 L25.6666667,54.8333333 L25.6666667,57.1666667 C25.6666667,58.4553311 24.6219977,59.5 23.3333333,59.5 L21,59.5 C19.7113356,59.5 18.6666667,58.4553311 18.6666667,57.1666667 L18.6666667,53.6670939 Z M22.1666667,16.3333333 C19.5893378,16.3333333 17.5,18.4226712 17.5,21 L17.5,35 C26.25,39.6666667 43.75,39.6666667 52.5,35 L52.5,21 C52.5,18.4226712 50.4106622,16.3333333 47.8333333,16.3333333 L22.1666667,16.3333333 Z M21,45.5 C19.7113356,45.5 18.6666667,46.5446689 18.6666667,47.8333333 C18.6666667,49.1219977 19.7113356,50.1666667 21,50.1666667 L25.3390942,50.1666667 C25.434483,50.1666667 25.5295117,50.1549681 25.6220524,50.1318329 C26.2471465,49.9755594 26.6272006,49.3421358 26.4709271,48.7170418 L25.8875938,46.3837084 C25.7577534,45.8643469 25.2911064,45.5 24.7557609,45.5 L21,45.5 Z M49,45.5 L45.2442391,45.5 C44.7088936,45.5 44.2422466,45.8643469 44.1124062,46.3837084 L43.5290729,48.7170418 C43.3727994,49.3421358 43.7528535,49.9755594 44.3779476,50.1318329 C44.4704883,50.1549681 44.565517,50.1666667 44.6609058,50.1666667 L49,50.1666667 C50.2886644,50.1666667 51.3333333,49.1219977 51.3333333,47.8333333 C51.3333333,46.5446689 50.2886644,45.5 49,45.5 Z M14,32.6666667 C11.6666667,31.5406659 10.5,30.7628881 10.5,30.3333333 L10.5,24.5 C10.5,23.2113356 11.5446689,22.1666667 12.8333333,22.1666667 L14,22.1666667 C14.6443322,22.1666667 15.1666667,22.6890011 15.1666667,23.3333333 L15.1666667,32.6666667 C15.1666667,33.3109989 15.6890011,33.8333333 16.3333333,33.8333333 L16.3333333,35 C15.0446689,35 14,33.9553311 14,32.6666667 Z M56,32.6666667 C56,33.9553311 54.9553311,35 53.6666667,35 L53.6666667,33.8333333 C54.3109989,33.8333333 54.8333333,33.3109989 54.8333333,32.6666667 L54.8333333,23.3333333 C54.8333333,22.6890011 55.3556678,22.1666667 56,22.1666667 L57.1666667,22.1666667 C58.4553311,22.1666667 59.5,23.2113356 59.5,24.5 L59.5,30.3333333 C59.5,30.7628881 58.3333333,31.5406659 56,32.6666667 Z M28.5833333,45.5 C28.2611672,45.5 28,45.7611672 28,46.0833333 C28,46.4054994 28.2611672,46.6666667 28.5833333,46.6666667 L41.4166667,46.6666667 C41.7388328,46.6666667 42,46.4054994 42,46.0833333 C42,45.7611672 41.7388328,45.5 41.4166667,45.5 L28.5833333,45.5 Z M29.75,47.8333333 C29.4278339,47.8333333 29.1666667,48.0945006 29.1666667,48.4166667 C29.1666667,48.7388328 29.4278339,49 29.75,49 L40.25,49 C40.5721661,49 40.8333333,48.7388328 40.8333333,48.4166667 C40.8333333,48.0945006 40.5721661,47.8333333 40.25,47.8333333 L29.75,47.8333333 Z M30.9166667,50.1666667 C30.5945006,50.1666667 30.3333333,50.4278339 30.3333333,50.75 C30.3333333,51.0721661 30.5945006,51.3333333 30.9166667,51.3333333 L39.0833333,51.3333333 C39.4054994,51.3333333 39.6666667,51.0721661 39.6666667,50.75 C39.6666667,50.4278339 39.4054994,50.1666667 39.0833333,50.1666667 L30.9166667,50.1666667 Z"
                      />
                    </g>
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>اتوبوس</p>
              </a>
            </li>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a
                href="/tour"
                data-test="tour-link"
                class="nuxt-link-exact-active nuxt-link-active router-link-active"
                data-v-fd004460
              >
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    height="70px"
                    width="70px"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    version="1.1"
                    viewBox="0 0 70 70"
                    class="svg-icon"
                    data-v-fd004460
                  >
                    <g stroke="none" stroke-width="1">
                      <g transform="translate(-565.000000, -367.000000)">
                        <g transform="translate(0.000000, 60.000000)">
                          <g transform="translate(200.000000, 312.000000)">
                            <g transform="translate(359.000000, 0.000000)">
                              <g transform="translate(6.000000, -5.000000)">
                                <path
                                  d="M35,68.8333333 C53.685634,68.8333333 68.8333333,53.685634 68.8333333,35 C68.8333333,16.314366 53.685634,1.16666667 35,1.16666667 C16.314366,1.16666667 1.16666667,16.314366 1.16666667,35 C1.16666667,53.685634 16.314366,68.8333333 35,68.8333333 Z M35,70 C15.6700338,70 0,54.3299662 0,35 C0,15.6700338 15.6700338,0 35,0 C54.3299662,0 70,15.6700338 70,35 C70,54.3299662 54.3299662,70 35,70 Z M14,43.1217826 L14,50.8742613 C14,51.8022481 14.7619262,52.5573819 15.6987454,52.5573819 L17.9118023,52.5573819 L17.9119525,39.2457666 C15.7547893,39.2456177 14,40.9844205 14,43.1217826 Z M52.0881977,39.2456177 L52.0881977,52.557233 L54.3014049,52.557233 C55.2379235,52.557233 56,51.8022481 56,50.8741125 L56,43.1216337 C55.9998497,40.9844205 54.2450605,39.2456177 52.0881977,39.2456177 Z M44.3675156,36.9502909 L44.3675156,33.7756619 L47.2884081,33.7756619 C48.4869657,33.7756619 49.6039366,33.4262339 50.5440613,32.8268315 L50.5440613,53.5647233 C50.5440613,54.9076466 49.4413643,56 48.0862445,56 L22.1194501,56 C20.76418,56 19.6617835,54.9074977 19.6617835,53.5647233 L19.6617835,32.8268315 C20.6019082,33.426085 21.7188791,33.775513 22.9172865,33.775513 L25.8380287,33.775513 L25.8380287,36.9502909 C25.8380287,37.5418024 26.3219893,38.0213541 26.9189392,38.0213541 C27.5158891,38.0213541 27.9998497,37.5418024 27.9998497,36.9502909 L27.9998497,33.775513 L42.2056945,33.775513 L42.2056945,36.9502909 C42.2056945,37.5418024 42.6896552,38.0213541 43.2866051,38.0213541 C43.883555,38.0213541 44.3675156,37.5418024 44.3675156,36.9502909 Z M47.841785,42.7647616 C47.841785,42.17325 47.3579746,41.6936984 46.7608745,41.6936984 L23.4448201,41.6936984 C22.8478702,41.6936984 22.3639095,42.17325 22.3639095,42.7647616 L22.3639095,51.5626956 C22.3639095,52.1542072 22.8478702,52.6337588 23.4448201,52.6337588 L46.7608745,52.6337588 C47.3578244,52.6337588 47.841785,52.1542072 47.841785,51.5626956 L47.841785,42.7647616 Z M24.5257306,43.8358248 L45.6799639,43.8358248 L45.6799639,50.4917813 L24.5257306,50.4917813 L24.5257306,43.8358248 Z M25.5812486,44.945002 L25.5812486,49.3826041 L44.6244459,49.3826041 L44.6244459,44.945002 L25.5812486,44.945002 Z M30.1614765,17.3275316 C30.9758684,15.3688303 32.8357762,14 34.9999249,14 C37.1640735,14 39.0239813,15.3688303 39.8383732,17.3275316 L47.2884081,17.3275316 C49.4240853,17.3275316 51.1615957,19.0492129 51.1618962,21.1654337 L51.1618962,27.7954846 C51.1618962,29.9117054 49.4243858,31.6333866 47.2887086,31.6333866 L44.3678161,31.6333866 L44.3678161,24.9397627 C44.3678161,24.3482512 43.8838555,23.8686995 43.2869056,23.8686995 C42.6899557,23.8686995 42.205995,24.3482512 42.205995,24.9397627 L42.205995,31.6333866 L28.0001503,31.6333866 L28.0001503,24.9397627 C28.0001503,24.3482512 27.5161896,23.8686995 26.9192397,23.8686995 C26.3222898,23.8686995 25.8383292,24.3482512 25.8383292,24.9397627 L25.8383292,31.6333866 L22.9172865,31.6333866 C20.7816092,31.6333866 19.0442491,29.9117054 19.0442491,27.7954846 L19.0442491,21.1654337 C19.0442491,19.0492129 20.7816092,17.3275316 22.9172865,17.3275316 L30.1614765,17.3275316 Z M31.3426875,17.3275316 L38.6571588,17.3275316 C37.9271427,16.0013963 36.5626876,15.1091772 34.9999249,15.1091772 C33.4371627,15.1091772 32.0727079,16.0013958 31.3426916,17.3275304 Z"
                                />
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>تور</p>
              </a>
            </li>
            <li class="col-xs-4 col-md-2" data-v-fd004460>
              <a href="/hotel" data-test="hotel-link" data-v-fd004460>
                <div class="svg-icon-wrapper" data-v-fd004460>
                  <svg
                    width="70px"
                    height="70px"
                    viewBox="0 0 70 70"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    data-v-fd004460
                  >
                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                      <g transform="translate(-465.000000, -407.000000)">
                        <g transform="translate(0.000000, 392.000000)">
                          <g transform="translate(275.000000, 15.000000)">
                            <g transform="translate(190.000000, 0.000000)">
                              <rect x="0" y="0" width="70" height="70" />
                              <path
                                d="M35,68.8333333 C53.685634,68.8333333 68.8333333,53.685634 68.8333333,35 C68.8333333,16.314366 53.685634,1.16666667 35,1.16666667 C16.314366,1.16666667 1.16666667,16.314366 1.16666667,35 C1.16666667,53.685634 16.314366,68.8333333 35,68.8333333 Z M35,70 C15.6700338,70 0,54.3299662 0,35 C0,15.6700338 15.6700338,0 35,0 C54.3299662,0 70,15.6700338 70,35 C70,54.3299662 54.3299662,70 35,70 Z M31.5,43.1666667 L31.5,31.5118804 C31.5,30.8609868 32.0036692,30.3333333 32.6249767,30.3333333 L35,30.3333333 L35,43.1666667 L37.3333333,43.1666667 L37.3333333,30.3333333 L48.8334574,30.3333333 C52.1470974,30.3333333 54.8333333,32.9716008 54.8333333,36.2260686 L54.8333333,54.25 C54.8333333,54.5721661 54.5721661,54.8333333 54.25,54.8333333 C53.9278339,54.8333333 53.6666667,54.5721661 53.6666667,54.25 L53.6666667,50.1666667 L16.3333333,50.1666667 L16.3333333,54.25 C16.3333333,54.5721661 16.0721661,54.8333333 15.75,54.8333333 C15.4278339,54.8333333 15.1666667,54.5721661 15.1666667,54.25 L15.1666667,29.75 C15.1666667,29.4278339 15.4278339,29.1666667 15.75,29.1666667 C16.0721661,29.1666667 16.3333333,29.4278339 16.3333333,29.75 L16.3333333,43.1666667 L31.5,43.1666667 Z M16.3333333,49 L53.6666667,49 L53.6666667,46.6666667 L16.3333333,46.6666667 L16.3333333,49 Z M23.9166667,40.8333333 C21.0171717,40.8333333 18.6666667,38.4828283 18.6666667,35.5833333 C18.6666667,32.6838384 21.0171717,30.3333333 23.9166667,30.3333333 C26.8161616,30.3333333 29.1666667,32.6838384 29.1666667,35.5833333 C29.1666667,38.4828283 26.8161616,40.8333333 23.9166667,40.8333333 Z M35.9919257,16.5518425 C35.3771242,16.2712083 34.6708476,16.2709225 34.0558192,16.551059 L14.1467306,25.6193589 C13.8886131,25.7369277 13.5839854,25.6242773 13.4644631,25.3670585 L13.464271,25.3666446 C13.3453476,25.1101163 13.4568983,24.8057525 13.7134266,24.6868291 L33.5497833,15.4909313 C34.4847031,15.0575137 35.5631135,15.0581288 36.4975383,15.4926126 L56.2889438,24.6951153 C56.5435961,24.8135222 56.6541547,25.115863 56.5359564,25.3706123 C56.4175724,25.6257618 56.1153977,25.7374639 55.8595188,25.6206646 L35.9919257,16.5518425 Z"
                                fill="currentColor"
                                fill-rule="nonzero"
                              />
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </div>
                <p class="tabs-title mb-1" data-v-fd004460>هتل خارجی و داخلی</p>
              </a>
            </li>
          </ul>
          <div class="row search-container" data-v-fd004460>
            <div class="gap"></div>
            <form action="#">
              <div class="col-xs-12 col-md-6 relative searchbar">
                <!---->
                <div class="input-pair">
                  <div class="input-group multitab-picker" data-v-7099b782>
                    <span class="input-group-addon">
                      <i class="icon icon-location" data-v-7099b782></i>
                    </span>
                    <div class="multitab-picker__dropdown v-dropdown" data-v-7099b782>
                      <ul class="multitab-picker__tabs" data-v-7099b782>
                        <li data-v-7099b782>
                          <a href="#" data-v-7099b782>
                            شهرهای پر
                            تردد
                          </a>
                        </li>
                      </ul>
                      <ul class="multitab-picker__items pretty-scroll" data-v-7099b782></ul>
                      <div class="text-center py-2" data-v-7099b782>موردی یافت نشد.</div>
                      <div data-v-7099b782></div>
                    </div>
                    <input
                      type="text"
                      placeholder="مبدا"
                      tabindex="1"
                      class="form-control"
                      data-v-7099b782
                    />
                  </div>
                  <div is-destination="true" class="input-group multitab-picker" data-v-ef5e3d2a>
                    <span class="input-group-addon">
                      <i class="icon icon-location" data-v-ef5e3d2a></i>
                    </span>
                    <div class="multitab-picker__dropdown v-dropdown" data-v-ef5e3d2a>
                      <!---->
                      <ul class="multitab-picker__items pretty-scroll" data-v-ef5e3d2a></ul>
                      <div class="text-center py-2" data-v-ef5e3d2a>موردی یافت نشد.</div>
                      <div data-v-ef5e3d2a></div>
                    </div>
                    <input
                      type="text"
                      placeholder="مقصد"
                      tabindex="2"
                      class="form-control"
                      data-v-ef5e3d2a
                    />
                  </div>
                </div>
              </div>
              <div class="col-xs-12 col-md-2 search-date">
                <div
                  data-test="alibabaDatepicker"
                  shortcuts
                  selectedIndex="-1"
                  jalaali="true"
                  class="alibaba-datepicker alibaba-datepicker--shortcuts"
                  data-v-65b601f8
                >
                  <div class="input-group" data-v-65b601f8>
                    <span class="input-group-addon" data-v-65b601f8>
                      <i slot="icon" class="icon icon-calendar" data-v-65b601f8></i>
                    </span>
                    <input
                      type="text"
                      placeholder="زمان تور"
                      tabindex="3"
                      value
                      class="form-control"
                      data-v-65b601f8
                    />
                  </div>
                  <div class="clearfix" data-v-65b601f8></div>
                  <!---->
                  <div class="alibaba-datepicker__wrapper v-dropdown fade" data-v-65b601f8>
                    <aside
                      class="alibaba-datepicker__sidebar font-13 w-normal text-darker"
                      data-v-65b601f8
                    >
                      <h6 class="mt-0 pb-2 font-13 w-normal" data-v-65b601f8>بازه های پر تردد</h6>
                      <div
                        class="text-center mt-3"
                        data-v-65b601f8
                      >لطفا ابتدا مبدا و مقصد را انتخاب کنید.</div>
                    </aside>
                    <div class="flex-1 relative" data-v-65b601f8>
                      <header class="alibaba-datepicker__header rtl" data-v-65b601f8>
                        <div data-v-65b601f8>
                          <button
                            data-test="datepickerChangeMode"
                            type="button"
                            class="btn btn-raw active"
                            data-v-65b601f8
                          >
                            <span class="pull-right" data-v-65b601f8>
                              تغییر به
                              میلادی
                            </span>
                          </button>
                          <span
                            data-test="datepickerMode"
                            class="w-normal px-2 text-center"
                            data-v-65b601f8
                          >تقویم شمسی</span>
                          <span data-v-65b601f8>
                            <button
                              data-test="datepickerGoToday"
                              type="button"
                              class="btn btn-raw btn-today hidden-xs pull-left"
                              data-v-65b601f8
                            >برو به امروز</button>
                            <button
                              data-test="datepickerClose"
                              type="button"
                              class="close visible-xs"
                              data-v-65b601f8
                            >
                              <span class="svg-icon" data-v-65b601f8>
                                <svg
                                  width="17px"
                                  height="17px"
                                  viewBox="0 0 17 17"
                                  version="1.1"
                                  xmlns="http://www.w3.org/2000/svg"
                                  xmlns:xlink="http://www.w3.org/1999/xlink"
                                  data-v-65b601f8
                                >
                                  <g
                                    stroke="none"
                                    stroke-width="1"
                                    fill="none"
                                    fill-rule="evenodd"
                                  >
                                    <g transform="translate(-25.000000, -18.000000)">
                                      <g transform="translate(0.000000, 1.000000)">
                                        <g transform="translate(0.000000, 15.000000)">
                                          <g transform="translate(25.000000, 2.000000)">
                                            <rect
                                              x="0"
                                              y="0"
                                              width="16.6666667"
                                              height="16.6666667"
                                            />
                                            <path
                                              d="M9.0699029,8.33333333 L16.5141181,15.7775486 C16.7175162,15.9809466 16.7175162,16.31072 16.5141181,16.5141181 C16.31072,16.7175162 15.9809466,16.7175162 15.7775486,16.5141181 L8.33333333,9.0699029 L0.889118115,16.5141181 C0.685720047,16.7175162 0.35594662,16.7175162 0.152548551,16.5141181 C-0.0508495172,16.31072 -0.0508495172,15.9809466 0.152548551,15.7775486 L7.59676377,8.33333333 L0.152548551,0.889118115 C-0.0508495172,0.685720047 -0.0508495172,0.35594662 0.152548551,0.152548551 C0.35594662,-0.0508495172 0.685720047,-0.0508495172 0.889118115,0.152548551 L8.33333333,7.59676377 L15.7775486,0.152548551 C15.9809466,-0.0508495172 16.31072,-0.0508495172 16.5141181,0.152548551 C16.7175162,0.35594662 16.7175162,0.685720047 16.5141181,0.889118115 L9.0699029,8.33333333 Z"
                                              fill="#9E9E9E"
                                              fill-rule="nonzero"
                                            />
                                          </g>
                                        </g>
                                      </g>
                                    </g>
                                  </g>
                                </svg>
                              </span>
                            </button>
                          </span>
                        </div>
                        <!---->
                      </header>
                      <!---->
                      <div class="alibaba-datepicker__arrows hidden-xs" data-v-65b601f8>
                        <button
                          type="button"
                          disabled="disabled"
                          data-test="datepickerPreviousMonth"
                          class="btn"
                          data-v-65b601f8
                        >
                          <svg
                            width="10px"
                            height="9px"
                            fill="currentColor"
                            viewBox="0 0 10 9"
                            version="1.1"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink"
                            class="block"
                            data-v-65b601f8
                          >
                            <g stroke="none" stroke-width="1" fill-rule="evenodd">
                              <g
                                transform="translate(-891.000000, -614.000000)"
                                fill-rule="nonzero"
                              >
                                <g transform="translate(376.000000, 547.000000)">
                                  <g transform="translate(510.000000, 62.000000)">
                                    <path
                                      d="M11.6604348,5.23588598 L14.8336263,9.06309507 L14.7652174,8.99686061 C14.7962499,9.02278001 14.8242801,9.05097896 14.8492665,9.08101169 L14.8720021,9.11029229 L14.8720021,9.11029229 L14.885,9.13 L14.8924536,9.13923358 C14.8996065,9.15009347 14.9064091,9.16113092 14.9128598,9.17232874 L14.9176246,9.18193403 L14.9176246,9.18193403 C14.9253373,9.19534632 14.9324305,9.20907617 14.9389856,9.22308616 C14.944727,9.23497938 14.94989,9.24703455 14.9546797,9.25921274 L14.9497106,9.24737076 C14.9537629,9.25811158 14.958005,9.2691225 14.9619366,9.28021093 C14.9694339,9.30014871 14.975549,9.32045932 14.9806836,9.34116098 C14.9832691,9.35330177 14.9859583,9.36565399 14.988286,9.3780534 C14.9921992,9.39665421 14.9949129,9.4156006 14.9968024,9.43479169 C14.9976374,9.44744792 14.998502,9.45985136 14.9990161,9.47226028 L14.9998017,9.48595501 L14.9998017,9.48595501 L15,9.50009883 L14.9999909,9.52178254 C14.9995878,9.53419019 14.9988339,9.54659438 14.9977276,9.55897904 L15,9.50009883 C15,9.54607857 14.9952888,9.5909447 14.9863242,9.63423773 L14.9851635,9.64085801 C14.9832844,9.64602709 14.9804434,9.6580185 14.9772653,9.66993056 C14.9714389,9.69412895 14.9636141,9.71786664 14.9545197,9.74095911 C14.9514858,9.74713603 14.9489078,9.75340808 14.9462302,9.75964438 C14.9366357,9.78332785 14.9251226,9.80623728 14.9123599,9.82832685 L14.8985385,9.85030904 L14.8985385,9.85030904 L14.8727875,9.88848956 C14.8652421,9.89870988 14.8573568,9.90877275 14.8491304,9.91866384 L11.6608696,13.7643117 C11.5321739,13.9196914 11.3465217,14 11.1591304,14 C11.0121739,14 10.8643478,13.9502436 10.7426087,13.8485484 C10.4656522,13.6172247 10.4278261,13.2043337 10.6586957,12.9267452 L12.956,10.154 L5.65217391,10.1547886 C5.29173913,10.1547886 5,9.86192405 5,9.50009883 C5,9.1382736 5.29173913,8.84540904 5.65217391,8.84540904 L12.956,8.845 L10.6582609,6.07345246 C10.4278261,5.79542752 10.4656522,5.3825365 10.7421739,5.15164923 C11.0191304,4.91988904 11.4304348,4.95829751 11.6604348,5.23588598 Z M14.8897045,9.13509429 L14.885,9.13 L14.8717517,9.10962825 C14.8779571,9.11799734 14.8839416,9.12648891 14.8897045,9.13509429 Z"
                                    />
                                  </g>
                                </g>
                              </g>
                            </g>
                          </svg>
                        </button>
                        <button
                          type="button"
                          disabled="disabled"
                          data-test="datepickerNextMonth"
                          class="btn"
                          data-v-65b601f8
                        >
                          <svg
                            width="10px"
                            height="9px"
                            fill="currentColor"
                            viewBox="0 0 10 9"
                            version="1.1"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink"
                            class="mirror-x block"
                            data-v-65b601f8
                          >
                            <g stroke="none" stroke-width="1" fill-rule="evenodd">
                              <g
                                transform="translate(-891.000000, -614.000000)"
                                fill-rule="nonzero"
                              >
                                <g transform="translate(376.000000, 547.000000)">
                                  <g transform="translate(510.000000, 62.000000)">
                                    <path
                                      d="M11.6604348,5.23588598 L14.8336263,9.06309507 L14.7652174,8.99686061 C14.7962499,9.02278001 14.8242801,9.05097896 14.8492665,9.08101169 L14.8720021,9.11029229 L14.8720021,9.11029229 L14.885,9.13 L14.8924536,9.13923358 C14.8996065,9.15009347 14.9064091,9.16113092 14.9128598,9.17232874 L14.9176246,9.18193403 L14.9176246,9.18193403 C14.9253373,9.19534632 14.9324305,9.20907617 14.9389856,9.22308616 C14.944727,9.23497938 14.94989,9.24703455 14.9546797,9.25921274 L14.9497106,9.24737076 C14.9537629,9.25811158 14.958005,9.2691225 14.9619366,9.28021093 C14.9694339,9.30014871 14.975549,9.32045932 14.9806836,9.34116098 C14.9832691,9.35330177 14.9859583,9.36565399 14.988286,9.3780534 C14.9921992,9.39665421 14.9949129,9.4156006 14.9968024,9.43479169 C14.9976374,9.44744792 14.998502,9.45985136 14.9990161,9.47226028 L14.9998017,9.48595501 L14.9998017,9.48595501 L15,9.50009883 L14.9999909,9.52178254 C14.9995878,9.53419019 14.9988339,9.54659438 14.9977276,9.55897904 L15,9.50009883 C15,9.54607857 14.9952888,9.5909447 14.9863242,9.63423773 L14.9851635,9.64085801 C14.9832844,9.64602709 14.9804434,9.6580185 14.9772653,9.66993056 C14.9714389,9.69412895 14.9636141,9.71786664 14.9545197,9.74095911 C14.9514858,9.74713603 14.9489078,9.75340808 14.9462302,9.75964438 C14.9366357,9.78332785 14.9251226,9.80623728 14.9123599,9.82832685 L14.8985385,9.85030904 L14.8985385,9.85030904 L14.8727875,9.88848956 C14.8652421,9.89870988 14.8573568,9.90877275 14.8491304,9.91866384 L11.6608696,13.7643117 C11.5321739,13.9196914 11.3465217,14 11.1591304,14 C11.0121739,14 10.8643478,13.9502436 10.7426087,13.8485484 C10.4656522,13.6172247 10.4278261,13.2043337 10.6586957,12.9267452 L12.956,10.154 L5.65217391,10.1547886 C5.29173913,10.1547886 5,9.86192405 5,9.50009883 C5,9.1382736 5.29173913,8.84540904 5.65217391,8.84540904 L12.956,8.845 L10.6582609,6.07345246 C10.4278261,5.79542752 10.4656522,5.3825365 10.7421739,5.15164923 C11.0191304,4.91988904 11.4304348,4.95829751 11.6604348,5.23588598 Z M14.8897045,9.13509429 L14.885,9.13 L14.8717517,9.10962825 C14.8779571,9.11799734 14.8839416,9.12648891 14.8897045,9.13509429 Z"
                                    />
                                  </g>
                                </g>
                              </g>
                            </g>
                          </svg>
                        </button>
                      </div>
                      <div class="alibaba-datepicker__container slide-left" data-v-65b601f8></div>
                      <hr class="hidden-xs mb-1 mt-0" data-v-65b601f8 />
                      <footer class="alibaba-datepicker__footer rtl" data-v-65b601f8>
                        <div data-v-65b601f8>
                          <p
                            class="text-black w-500 font-12 m-0 hidden-xs"
                            data-v-65b601f8
                          >تاریخ انتخابی شما:</p>
                          <em data-v-65b601f8>---</em>
                        </div>
                        <div data-v-65b601f8>
                          <button
                            type="button"
                            disabled="disabled"
                            class="btn btn-block btn-orange py-1 w-500"
                            style="min-width: 90px;"
                            data-v-65b601f8
                          >تایید</button>
                        </div>
                      </footer>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-xs-12 col-md-2 search-date">
                <div class="input-group" data-v-03ced2b4>
                  <span class="input-group-addon" data-v-03ced2b4>
                    <i class="icon icon-passenger" data-v-03ced2b4></i>
                  </span>
                  <div class="v-dropdown mt-1" data-v-03ced2b4>
                    <div class="p-2 clearfix" data-v-03ced2b4>
                      <div class="search-passengers-room" data-v-cb81412c data-v-03ced2b4>
                        <div class="p-2" data-v-cb81412c>
                          <!---->
                          <h5 class="inline text-orange w-500" data-v-cb81412c>اتاق اول</h5>
                        </div>
                        <div class="p-1 clearfix" data-v-cb81412c>
                          <strong class="w-500 ml-2" data-v-cb81412c>بزرگسال</strong>
                          <small class="text-gray" data-v-cb81412c>(12 سال به بالا)</small>
                          <div class="pull-left" data-v-cb81412c>
                            <span class="style-btn-increase inline-block" data-v-cb81412c></span>
                            <span
                              class="inline-block mx-1 text-center"
                              style="min-width: 12px;"
                              data-v-cb81412c
                            >2</span>
                            <span class="style-btn-decrease inline-block" data-v-cb81412c></span>
                          </div>
                        </div>
                        <div class="p-1 clearfix" data-v-cb81412c>
                          <strong class="w-500 ml-2" data-v-cb81412c>کودک</strong>
                          <small class="text-gray" data-v-cb81412c>(0 سال تا 12 سال)</small>
                          <div class="pull-left" data-v-cb81412c>
                            <span class="style-btn-increase inline-block" data-v-cb81412c></span>
                            <span
                              class="inline-block mx-1 text-center"
                              style="min-width: 12px;"
                              data-v-cb81412c
                            >0</span>
                            <span class="style-btn-decrease inline-block" data-v-cb81412c></span>
                          </div>
                        </div>
                        <!---->
                      </div>
                      <hr class="my-0" data-v-03ced2b4 />
                      <div class="m-2 mt-3" data-v-03ced2b4>
                        <button
                          type="button"
                          class="btn btn-primary btn-outline btn-block"
                          data-v-03ced2b4
                        >
                          <i class="icon-plus v-middle" data-v-03ced2b4></i>
                          اضافه کردن اتاق
                        </button>
                      </div>
                    </div>
                  </div>
                  <input
                    type="text"
                    name="passengers"
                    placeholder="مسافران"
                    tabindex="4"
                    readonly="readonly"
                    aria-describedby="search-passengers"
                    value="2 بزرگسال ، 1 اتاق"
                    class="form-control"
                    data-v-03ced2b4
                  />
                </div>
              </div>
              <div class="col-xs-12 col-md-2">
                <button
                  type="submit"
                  tabindex="4"
                  class="btn btn-lg btn-orange btn-block"
                  data-v-3ad51da2
                >جستجو</button>
              </div>
              <div class="clearfix"></div>
            </form>
          </div>
        </div>
      </div>
    </section>
    <div data-v-35dc4fe5>
      <div id="promotions" class="mt-3 container" data-v-e7295378 data-v-35dc4fe5>
        <div class="flex flex-wrap flex-justify-center flex-align-center row" data-v-e7295378></div>
      </div>
      <!-- <section class="container text-center text-black py-3" data-v-35dc4fe5>
                            <div class="row">
                                <div class="col-xs-4 col-md-4"><img src="/images/cdn/b4d1d1c.svg"
                                        alt="سرویس‌های علی بابا -‌ مشاوره و پشتیبانی" width="140" class="center-block">
                                    <h4 class="font-14">
                                        مشاوره و پشتیبانی تخصصی رایگان
                                    </h4>
                                    <p>مشاوره قبل و حین سفر توسط سفریار اختصاصی</p>
                                </div>
                                <div class="col-xs-4 col-md-4"><img src="/images/cdn/193a210.svg"
                                        alt="سرویس‌های علی بابا -‌ قیمت رقابتی تور" width="140" class="center-block">
                                    <h4 class="font-14">
                                        قیمت رقابتی تورهای داخلی و خارجی
                                    </h4>
                                    <p>قیمت‌های شفاف بدون پرداخت هزینه‌های جانبی</p>
                                </div>
                                <div class="col-xs-4 col-md-4"><img src="/images/cdn/115666d.svg"
                                        alt="سرویس‌های علی بابا -‌ تنوع تورها" width="140" class="center-block">
                                    <h4 class="font-14">

                                        تنوع تورهای داخلی و خارجی
                                    </h4>
                                    <p>شخصی سازی انواع تور برای مسافران</p>
                                </div>
                            </div>
      </section>-->
      <div class="container text-black text-center my-3 py-2">
        <div class="row">
          <div class="col-xs-4 col-md-4">
            <img
              src="/images/cdn/b4d1d1c.svg"
              alt="سرویس‌های علی بابا -‌ مشاوره و پشتیبانی"
              width="140"
              class="center-block"
            />
            <h4 class="font-14">مشاوره و پشتیبانی تخصصی رایگان</h4>
            <p>مشاوره قبل و حین سفر توسط سفریار اختصاصی</p>
          </div>
          <div class="col-xs-4 col-md-4">
            <img
              src="/images/cdn/193a210.svg"
              alt="سرویس‌های علی بابا -‌ قیمت رقابتی تور"
              width="140"
              class="center-block"
            />
            <h4 class="font-14">قیمت رقابتی تورهای داخلی و خارجی</h4>
            <p>قیمت‌های شفاف بدون پرداخت هزینه‌های جانبی</p>
          </div>
          <div class="col-xs-4 col-md-4">
            <img
              src="/images/cdn/115666d.svg"
              alt="سرویس‌های علی بابا -‌ تنوع تورها"
              width="140"
              class="center-block"
            />
            <h4 class="font-14">تنوع تورهای داخلی و خارجی</h4>
            <p>شخصی سازی انواع تور برای مسافران</p>
          </div>
        </div>
      </div>

      <div class="headline text-center mb-2 text-darker">
        <h1 class="w-300 m-0">رزرو هتل</h1>
        <br />
        <h2 class="w-500 m-0">هتل های خارجی</h2>
      </div>
      <section data-v-f1c6aa14 class="c-popular-hotel-section mb-3">
        <div data-v-f1c6aa14 class="container">
          <div data-v-f1c6aa14 class="badge-container text-center mt-2">
            <a
              data-v-f1c6aa14
              @click="showHotel('istanbul')"
              class="badge badge--outline mx-1"
              :class="this.city=='istanbul'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">استانبول</small>
            </a>
            <a
              data-v-f1c6aa14
              @click="showHotel('dubai')"
              class="badge badge--outline mx-1"
              :class="this.city=='dubai'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">دبی</small>
            </a>
            <a
              data-v-f1c6aa14
              @click="showHotel('paris')"
              class="badge badge--outline mx-1"
              :class="this.city=='paris'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">پاریس</small>
            </a>
            <a
              data-v-f1c6aa14
              @click="showHotel('ankara')"
              class="badge badge--outline mx-1"
              :class="this.city=='ankara'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">آنکارا</small>
            </a>
            <a
              data-v-f1c6aa14
              @click="showHotel('baku')"
              class="badge badge--outline mx-1"
              :class="this.city=='baku'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">باکو</small>
            </a>
            <a
              data-v-f1c6aa14
              @click="showHotel('tblisi')"
              class="badge badge--outline mx-1"
              :class="this.city=='tblisi'?'badge--orange':''"
            >
              <small data-v-f1c6aa14 class="w-300">تفلیس</small>
            </a>
          </div>
          <div data-v-f1c6aa14 class="row">
            <div
              data-v-f1c6aa14
              v-for="hotel in this.hotels"
              :key="hotel._id"
              class="col-xs-12 col-sm-6 col-md-4"
            >
              <div data-v-a62baf82 data-v-f1c6aa14 class="c-popular-hotel-card">
                <a data-v-a62baf82 rel="nofollow">
                  <div
                    data-v-a62baf82
                    class="card-image"
                    :style="'background-image: url('+hotel.image+');'"
                  >
                    <div data-v-a62baf82 class="card-overlay" v-if="hotel.rate_number>0">
                      <div data-v-a62baf82 class="card-rate">
                        <div data-v-a62baf82 class="rate-number font-16">{{hotel.rate_number}}</div>
                        <div data-v-a62baf82 class="rate-text font-10">{{hotel.rate_desc}}</div>
                      </div>
                    </div>
                  </div>

                  <div data-v-a62baf82 class="card-content">
                    <div
                      data-v-a62baf82
                      class="flex flex-justify-space-between flex-align-center rtl font-en"
                    >
                      <p
                        data-v-a62baf82
                        class="text-ellipsis text-darker w-500 font-16 m-0"
                      >{{ hotel.name }}</p>
                      <div data-v-02a24b22 data-v-a62baf82 class="rate-stars">
                        <i
                          data-v-02a24b22
                          class="icon-star-rate"
                          :class="hotel.stars>=1?'active':''"
                        ></i>
                        <i
                          data-v-02a24b22
                          class="icon-star-rate"
                          :class="hotel.stars>=2?'active':''"
                        ></i>
                        <i
                          data-v-02a24b22
                          class="icon-star-rate"
                          :class="hotel.stars>=3?'active':''"
                        ></i>
                        <i
                          data-v-02a24b22
                          class="icon-star-rate"
                          :class="hotel.stars>=4?'active':''"
                        ></i>
                        <i
                          data-v-02a24b22
                          class="icon-star-rate"
                          :class="hotel.stars>=5?'active':''"
                        ></i>
                      </div>
                    </div>
                    <hr data-v-a62baf82 class="mt-2" />
                    <div data-v-a62baf82 class="flex flex-justify-space-between flex-align-center">
                      <div data-v-a62baf82>
                        <div data-v-a62baf82 class="text-gray w-normal">شروع قیمت از</div>
                        <strong
                          data-v-a62baf82
                          class="text-blue"
                        >{{hotel.price.toLocaleString()}} ریال</strong>
                      </div>                        <nuxt-link :to="'/Hotel/'+hotel._id">

                      <button data-v-a62baf82 class="btn btn-primary content-btn mt-3">
                        مشاهده و
                        رزرو
                      </button></nuxt-link>
                    </div>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      form: "login",
      hotels: [{"_id":1,"name":"Antik Istanbul","stars":4,"price":5612000,"image":"https://cdn.alibaba.ir/inh/images/hotel/4461/113287944.jpg","rate_number":8.3,"rate_desc":"بسیار عالی"},{"_id":0,"name":"Akgün Istanbul","stars":5,"price":0,"image":"https://cdn.alibaba.ir/inh/images/hotel/4460/01c73e53.jpg","rate_number":0,"rate_desc":""},{"_id":2,"name":"Black Bird Hotel- Istanbul","stars":4,"price":4554000,"image":"https://cdn.alibaba.ir/inh/images/hotel/4463/01e9d4fd.jpg","rate_number":-1,"rate_desc":""},{"_id":3,"name":"Carlton Hotel","stars":4,"price":5507000,"image":"https://cdn.alibaba.ir/inh/images/hotel/4464/0a56b477.jpg","rate_number":-1,"rate_desc":""},{"_id":4,"name":"Intercontinental Istanbul","stars":5,"price":12063000,"image":"https://cdn.alibaba.ir/inh/images/hotel/4465/004b51a5.jpg","rate_number":9,"rate_desc":"فوق العاده"},{"_id":5,"name":"Conrad Istanbul Bosphorus","stars":5,"price":11991000,"image":"https://cdn.alibaba.ir/inh/images/hotel/4466/00421bc2.jpg","rate_number":-1,"rate_desc":""}],
      city: "istanbul"
    };
  },
  methods: {
    async showHotel(name) {
      let headers = {
        //Authorization: "bearer " + state.user.token,
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json"
      };
      let url = "http://37.152.185.50:8080/hotels/" + name;
      await this.$axios({
        method: "get",
        headers: headers,
        url: url
      })
        .then(data => {
          this.hotels = data.data;
          this.city = name;
        })
        .catch(e => console.log(e));
    }
  }
};
</script>